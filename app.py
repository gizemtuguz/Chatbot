...
from flask_cors import CORS                     #farklı kaynaklardan gelen istekleri yonetmek için
from chatbot_application import get_chatbot_response, df_original
import requests
import base64

app = Flask(__name__)
CORS(app)                   #uygulamada corsu etkinleştirip istekleri kabule der

#resim urlsini indirip base64 formatında döndüren yardımcı fonksiyon
def fetch_image_as_base64(image_url):
    try:
        response = requests.get(image_url)              #urlnin içindeki resmi indirir
        response.raise_for_status()  #hata oluşursa yakalar
